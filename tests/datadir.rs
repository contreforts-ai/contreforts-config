...
//! (`crates/contreforts-core/src/config.rs`): explicit env override -> per-user
//! OS data dir -> a relative fallback -- with a distinct env var and leaf
//! directory so the two stores never resolve to the same path.
...
fn per_user_default_is_under_the_os_data_dir() {
    ...
    let def = ConfigStoreConfig::per_user_default();
    assert_eq!(
        def,
...
fn per_user_default_differs_from_the_knowledge_graph_store_path() {
    ...
    let config_store_path = ConfigStoreConfig::per_user_default();
